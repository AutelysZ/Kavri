...
- **Framework-independent.** Core design has no dependency on Express, Fastify, etc. The web module
  produces a standard `(req, res) => void` handler usable with `node:http`, Bun, Deno, or any
  adapter.
- **Parsed input only.** Handlers receive validated, typed data — not raw streams. Body parsing
  happens before handlers and interceptors see the request (gRPC-style).
- **Single interception mechanism.** Interceptors replace middleware, guards, pipes, and filters.
  One abstraction, one chain.
- **Controllers are singletons.** Per-request data lives in `Context` — a typed key-value store from
  `@kavri/basic`.
- **Route-first.** All endpoints are defined via `defineRoute()` in `@kavri/schema`. Controllers
  implement routes via `@Controller(route)` + `implements ControllerType<typeof route>`.
...
// Special response utils (bypass normal JSON serialization)
class Redirect {
...
Normal handler return → JSON serialized with 200. Return `Redirect`, `FileResponse`,
`StreamResponse`, or `RawResponse` for non-standard responses.

## 3. RequestContext & Request Keys

`AsyncScope` and `Key<T>` live in `@kavri/basic`. The web module creates its own `RequestContext`
instance and defines typed keys for request-scoped state. WebSocket connections also use
`RequestContext` since upgrades are HTTP requests.
...
Controllers are the sole mechanism for implementing HTTP endpoints. Every controller implements a
route definition from `@kavri/schema`.
...
- `@Controller(route)` composes `@Component()` and registers all routing metadata from the route
  definition.
- `implements ControllerType<typeof route>` enforces that all endpoint methods are implemented with
  correct types. No `extends`/`override`/`super()` needed.
- Handlers receive parsed input. Return typed response or a special response object (`Redirect`,
  `FileResponse`, etc.).
- Access per-request data via `ctx.get(key)` / `ctx.getOrThrow(key)`.
- **Controllers should never be injected by application code.** They are discovered by the framework
  via `Metadata.entries(Controller)`.
...
abstract class Interceptor {
    ...
    static readonly VALIDATE = 9000;   // decode Params against request schema
...
Interceptors are `@Component()` classes extending `Interceptor`. Discovered via
`injectAll(Interceptor, 'priority')`. Sorted by `@Priority` value (smaller first). `next()` invokes
the next interceptor or the handler. Use built-in keys (`REQUEST`, `ENDPOINT`, `CONTROLLER`,
`PARAMS`) to access request data.

Use `@Priority(Interceptor.EXCEPTION)` to place an interceptor at the exception-handling stage.
Fine-tune with `+1`/`-1` if needed, but avoid unless necessary.
...
class BasicAuthInterceptor extends Interceptor {
    constructor(private readonly config = injectConfig(BasicAuthOptions)) {
        super();
    }
...
Uses [`expressjs/compression`](https://github.com/expressjs/compression) (peer dependency) to
transparently compress responses. Works by wrapping the raw `res` before downstream interceptors
write to it — compression happens at the stream level, handling all response types (JSON, streams,
files) automatically.
...
class CompressionInterceptor extends Interceptor {
    ...
    constructor(private readonly config = injectConfig(CompressionOptions)) {
        super();
    }
...
class CorsInterceptor extends Interceptor {
    constructor(private readonly config = injectConfig(CorsOptions)) {
        super();
    }
...
abstract class RateLimitStore {
    ...
    abstract increment(key: string, windowMs: number): Awaitable<{
        count: number;
        resetAt: number
    }>;
}
...
The cookie is **not** `HttpOnly` — frontend JS must read it to echo in the request header. Security
relies on same-origin policy: an attacker on a different origin can cause the browser to send the
cookie, but can't read its value to set the header.
...
class CsrfInterceptor extends Interceptor {
    constructor(private readonly config = injectConfig(CsrfOptions)) {
        super();
    }
...
Route contracts are defined in `@kavri/schema` using `defineRoute()` —
see [09-schema-design.md](./09-schema-design.md#7-service-definitions-protobuf-style). Shared
...
See [09-schema-design.md](./09-schema-design.md#openapi-generation). `toOpenAPI(route, options)` in
`@kavri/schema` generates OpenAPI 3.x from route definitions. Static — no running container needed.

## 8. Static Assets

Uses [`expressjs/serve-static`](https://github.com/expressjs/serve-static) (peer dependency).
Handles caching headers, range requests, directory index, etc.
...
class StaticFileInterceptor extends Interceptor {
    ...
    constructor(private readonly config = injectConfig(StaticOptions)) {
        super();
    }
...
See [11-transaction-design.md](./11-transaction-design.md). `@Transactional()`,`TransactionManager`,
`DataSourceDriver`, `DataSourceManager` — all in `@kavri/web`. ORM drivers in`@kavri/drizzle` and
`@kavri/sequelize`.

## 10. WebSocket

See [13-websocket-design.md](./13-websocket-design.md). `defineWebSocket()`,
`@WebSocketHandler(protocol)`, `WebSocketHandlerBase`, `HandlerType`, `ConnectionHub`,
`WebSocketCodec`.
...
@Component()
@Import(ResponseInterceptor, ExceptionInterceptor, RouteInterceptor,
...
class WebApplication {
    ...
    static async create(entrypoint: AnyConstructor): Promise<WebApplication> {
...
@Import(LoggingInterceptor, BasicAuthInterceptor)
@Inject(InfraModule)
class MyApplication {
}
...
All built-in interceptors are registered by the framework automatically. Users `@Import` their own
interceptors to insert into the chain.

### ResponseInterceptor (RESPONSE = 0)

The outermost interceptor. Awaits the result from the entire downstream chain and writes it to the
HTTP response.
...
Body parsing is split into specialized interceptors. Each handles one content type and skips if not
applicable. All share the same priority — only one activates per request based on `Content-Type` and
`requestType`.

All parse interceptors are **passive** — if the target key is already set (by a user interceptor
earlier in the chain), parsing is skipped. This allows custom parsing logic to take priority.
...
class JsonParseInterceptor extends Interceptor {
    constructor(private readonly config = injectConfig(WebOptions)) {
        super();
    }
...
Each interceptor is a `@Component()` — users can replace any parser by providing their own at a
higher priority, setting the key before the built-in parser runs.

### ResolveInterceptor (RESOLVE = 7000)

Merges raw pieces (PathParams, Query, Body, Files) into a single object matching the request schema
shape. Sets `Params`.
...
class ValidateInterceptor extends Interceptor {
    async intercept(ctx: Context, next: (ctx: Context) => unknown) {
        ...
        const params = ctx.get(PARAMS);
        const requestClass = endpoint.request as AnyConstructor;

        // Validate and parse using @kavri/schema
        const error = decode(requestClass, params);
        if (error) {
...
Dispatches to the matched controller action. If no HTTP endpoint matched, passes through to
`next()` (does **not** throw 404 — that's handled by the end of the chain).
...
class ActionInterceptor extends Interceptor {
    async intercept(ctx: Context, next: (ctx: Context) => unknown) {
        ...
        if (endpoint.response !== 'void' && endpoint.response !== 'stream' && result !== undefined) {
            const responseClass = endpoint.response as AnyConstructor;
            const error = decode(responseClass, result);
            if (error) {
...
Handles WebSocket upgrade requests. If the matched endpoint is a WebSocket protocol, performs the
upgrade. Otherwise passes through.
...
class WebSocketUpgradeInterceptor extends Interceptor {
    ...
    private async upgrade(req: IncomingMessage, res: ServerResponse, endpoint: any) {
        ...
        // 5. Wire inbound message → decode → decode → dispatch to handler method
    }
}
```
