...
/**
 * Parse JSON string to value, serialize value to JSON string.
 * No children schema — the parsed value's type is validated by
 * other decorators on the field.
 *
 * parse: `JSON.parse(value)`
 * serialize: `JSON.stringify(value)`
 */
export const JsonString: ((schema?: StringSchema) => SchemaFieldDecorator<StringSchema>) &
  SchemaFieldDecoratorFactoryStatic<StringSchema> = (() => {
  const factory = createSchemaFieldDecoratorFactory(
    (schema?: StringSchema): any => {
      return SchemaField(factory, (schema ?? {}) as any);
    },
    {
      ...
      toJsonSchema: () => ({
        ...
        // contentSchema is generated by the toJsonSchema utility from sibling decorators
      }),
    },
  );
  return factory;
