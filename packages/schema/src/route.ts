...
// ---------------------------------------------------------------------------
// OpenAPI generation (stub — full impl in Layer 3+)
// ---------------------------------------------------------------------------
...
export function generateOpenAPI(
  ...
  // Minimal stub — full implementation uses toJsonSchema per endpoint
  return {
    openapi: '3.1.0',
    info: { title: options.title, version: options.version },
    paths: Object.fromEntries(
      Object.entries(route.endpoints).map(([name, ep]) => {
        const e = ep as Endpoint;
        const fullPath = route.base + (e.path || `/${name}`);
        return [
          fullPath,
          {
            [e.method.toLowerCase()]: {
              operationId: name,
              ...(e.options.title ? { summary: e.options.title } : {}),
              ...(e.options.description ? { description: e.options.description } : {}),
              ...(e.options.tags ? { tags: e.options.tags } : {}),
